...
if (!function_exists('module_config')) {
     ...
     * @param string $name
     * @param string $value
     * @author caiweiming <liangtao.gz@foxmail.com>
     * @return mixed
     */
...
if (!function_exists('action_log')) {
     ...
     * @alter Taoge <liangtao.gz@foxmail.com>
     * @return bool|string
     */
...
if(!function_exists('lt')){
     ...
     * @param  bool $type 是否终止   默认1
     * @return string            字符串
     */
    function lt($value, $type = 1, $print = 0)
    {
        header("Content-type: text/html; charset=utf-8");
        if ($print == 1) {
            //调试日志
            $data = var_export($value, TRUE);
            file_put_contents('lt.log', "\r\n" . date('Y-m-d H:i:s') . "\r\n" . $data, FILE_APPEND);
        } else {
